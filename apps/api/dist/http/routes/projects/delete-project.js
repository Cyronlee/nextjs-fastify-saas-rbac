...
async function deleteProject(app) {
  app.withTypeProvider().register(auth).delete(
    "/organization/:slug/projects/:projectId",
    {
      schema: {
        ...
        params: import_zod13.default.object({
          ...
          projectId: import_zod13.default.string().uuid()
        }),
        response: {
    ...
    async (request, reply) => {
      ...
      if (!project) {
        throw new BadRequestError(errors2.projects.NOT_FOUND);
      }
      const authProject = projectSchema.parse(project);
      const { cannot } = getUserPermissions(userId, membership.role);
      if (cannot("delete", authProject)) {
        throw new UnauthorizedError(errors2.projects.CANNOT_DELETE);
      }
      const avatars = [];
