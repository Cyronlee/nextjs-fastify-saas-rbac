  ...
  app.withTypeProvider().register(auth).post(
    "/organization/:slug/invites",
    {
      schema: {
        ...
        response: {
          201: import_zod13.default.object({
            inviteId: import_zod13.default.string().uuid()
          })
        }
      }
    },
    async (request, reply) => {
      ...
      if (cannot("create", "Invite")) {
        throw new UnauthorizedError(errors2.organizations.invites.CANNOT_SEND);
      }
      const { email, role } = request.body;
      const domain = email.split("@").at(1);
      if (organization.shouldAttachUsersByDomain && organization.domain === domain) {
        throw new BadRequestError(
          errors2.organizations.invites.AUTOJOIN_DOMAIN.replace(
            "{domain}",
            domain
          )
        );
      ...
      if (inviteWithSameEmail) {
        throw new BadRequestError(errors2.organizations.invites.ALREADY_EXISTS);
      }
      ...
      if (memberWithSameEmail) {
        throw new BadRequestError(errors2.organizations.invites.ALREADY_MEMBER);
      }
